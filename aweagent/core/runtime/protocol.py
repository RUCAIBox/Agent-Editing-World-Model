...
# artefacts regardless of the configured runtime backend.
...
class RuntimeSession(ABC):
    ...
    async def apply_patch(self, cwd: str, patch: str) -> ExecutionResult:
        ...
        await self.upload_file("/tmp/_aweagent.patch", patch.encode())

        strategies = [
            ("git apply --verbose /tmp/_aweagent.patch", False),
            ("git apply --verbose --ignore-space-change --ignore-whitespace /tmp/_aweagent.patch", False),
            ("patch --batch --fuzz=5 -p1 -i /tmp/_aweagent.patch", False),
            ("git apply --verbose --reject /tmp/_aweagent.patch", True),
            ("git apply --verbose --reject --ignore-space-change --ignore-whitespace /tmp/_aweagent.patch", True),
            ("git apply --verbose --reject --ignore-space-change --ignore-whitespace --allow-empty /tmp/_aweagent.patch", True),
        ]
