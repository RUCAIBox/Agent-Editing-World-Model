...
from aweagent.core.agent.policy import LoopPolicy, NoOpPolicy
        ...
        for step in range(self.ctx.max_steps):
            ...
            try:
                ...
                if action.usage is not None:
                    prompt_tokens = getattr(action.usage, "prompt_tokens", 0) or 0
                    completion_tokens = getattr(action.usage, "completion_tokens", 0) or 0
        ...
        if not self.ctx.task_info.get("skip_patch_extraction"):
            try:
                ...
                if self.ctx.task_info.get("inject_gitignore_in_patch") is False:
                    from aweagent.core.runtime.protocol import (
...
def _make_tool_call_obj(tc: dict[str, Any]) -> Any:
    ...
    from aweagent.core.llm.types import ToolCall
    name = tc.get("name", tc.get("function", {}).get("name", ""))
