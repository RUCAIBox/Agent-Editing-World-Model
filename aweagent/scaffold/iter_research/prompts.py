"""IterResearch (v19) prompts and the XML/JSON output-format parsing helpers.

These are ported verbatim from the reference harness so the agent sees exactly
the prompts the model was trained on. The agent's working memory lives entirely
in the ``<report>`` it rewrites each turn; the harness reconstructs a single
fresh user message from {question, tools, last report, last action, last
observation} via :data:`instruction_prompt` (or :data:`last_instruction_prompt`
on the final turn) and discards everything else (Markovian state reconstruction).

Output contract enforced by :func:`check_report_action`: every assistant turn
must contain ``<report>...</report>`` and *either* a ``<tool_call>`` carrying a
JSON object ``{"name": ..., "arguments": {...}}`` *or* a final ``<answer>``.
"""
...
def extract_tags(text: str, tag: str) -> str:
    """Return the (stripped) content of the first ``<tag>...</tag>`` block, else ''."""
    pattern = rf"<{tag}>(.*?)</{tag}>"
...
def check_report_action(text: str) -> tuple[bool, str]:
    """Validate the IterResearch output format.

    A valid turn has a ``<report>`` AND at least one of: a ``<answer>``, or a
    ``<tool_call>`` whose payload parses to a JSON object containing ``arguments``.
    Returns ``(ok, reason)``.
    """
    ...
    if not action and not answer:
        return False, "answer or action not found"
    if not answer and not isinstance(tool_call, dict):
        return False, "found neither answer nor valid tool call"
    return True, "success"


# Placeholders filled in the per-turn prompt templates. Replaced in a single
# left-to-right pass (see :func:`render_template`) so that inserted content —
# notably the observation, which is arbitrary tool output — is never re-scanned
# for further placeholders.
_PLACEHOLDERS = ("{question}", "{report}", "{action}", "{observation}", "{tools}", "{date_to_use}")
_PLACEHOLDER_RE = re.compile("|".join(re.escape(p) for p in _PLACEHOLDERS))


def render_template(template: str, mapping: dict[str, str]) -> str:
    """Fill ``{...}`` placeholders in one pass; unmatched values default to ''."""
    return _PLACEHOLDER_RE.sub(lambda m: mapping.get(m.group(0), ""), template)
