"""ScaleSWE prompt templates.

System prompt: OpenHands-style system prompt.
User prompt: standard SWE-bench format.
"""
...
#
# Modifications from the OpenHands original:
#   - Removed trailing {function_call_prompt} placeholder.
#     AweAgent injects tool schemas via SearchSWEAgent.get_system_prompt()
#     → format.get_system_prompt_suffix(), so the placeholder is not needed.
...
# ── User prompt (SWE-bench standard format) ───────────────────────────────────

SCALESWE_PROMPT = """We are addressing the following issue in our repository. Please review the issue details below:
...
SYSTEM_PROMPTS: dict[str, str] = {
    "openhands": OPENHANDS_SYSTEM_PROMPT,
}

USER_PROMPTS: dict[str, str] = {
    "scaleswe": SCALESWE_PROMPT,
}
