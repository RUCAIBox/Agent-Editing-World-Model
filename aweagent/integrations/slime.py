...
def _get_runtime_config() -> RuntimeConfig:
    """Build runtime config from environment, using Docker by default."""
    ...
    return RuntimeConfig(backend=backend, image=image, timeout=timeout)
...
def generate_rollout_fully_async(
    ...
    - ``AWE_AGENT_RUNTIME_BACKEND``: Registered runtime backend (default ``docker``).
